...
class ActivityNameLearningPath extends LocalizeFoundationName(HypermediaStateMixin(LitElement)) {
	...
	render() {
		return html`
			${this._hasAction('updateName') ? html`<d2l-input-text @input="${this._onInputName}" label="Name" placeholder=${this.localize('action.name')} value="${this.name}"></d2l-input-text>` : null}
