...
export default {
	...
	noActivitiesName: '{firstName} n’a aucune activité avec des dates d’échéance ou de fin disponibles.', // 'Empty state' - When widget has no activities in full page view, when we want to show the user's name instead of referring to them as "you"
	noActivitiesNameless: 'Il n’y a aucune activité avec des dates d’échéance ou de fin disponibles.', // 'Empty state' - When widget has no activities in full page view, when we want to refer to the user in the third person but don't know their name
	...
	noActivitiesFutureActivitiesName: "{firstName} n’a aucune activité à remettre ou se terminant bientôt. Revenez plus tard ou cliquez sur Afficher tout le travail pour voir les activités à venir.", // Shown under the same conditions as noActivitiesFutureActivities, when we want to show the user's name instead of referring to them as "you"
	noActivitiesFutureActivitiesNameless: "Il n’y a aucune activité à remettre ou se terminant bientôt. Revenez plus tard ou cliquez sur Afficher tout le travail pour voir les activités à venir.", // Shown under the same conditions as noActivitiesFutureActivities, when we want to refer to the user in the third person but don't know their name
	...
	noActivitiesNoFutureActivitiesName: '{firstName} n’a aucune activité avec des dates d’échéance ou de fin disponibles. Revenez plus tard pour vérifier si {firstName} a du travail à faire.', // Shown under the same conditions as noActivitiesNoFutureActivities, when we want to show the user's name instead of referring to them as "you"
	noActivitiesNoFutureActivitiesNameless: 'Il n’y a aucune activité avec des dates d’échéance ou de fin disponibles. Revenez plus tard pour vérifier s’il y a du travail à faire.', // Shown under the same conditions as noActivitiesNoFutureActivities, when we want to refer to the user in the third person but don't know their name
