...
class W2dCollections extends LocalizeDynamicMixin(HypermediaStateMixin(LitElement)) {
	...
	render() {
		...
		if (!this.skeleton && !this.loading && this._overdue.length === 0 && this._categories.length === 0) {
			return html`
		...
		return html`
			${this.loading || this.skeleton ? html`<div class="fullscreen">Loading</div>` : null}
	...
	get _pageUpcoming() {
		return typeof this._page === 'number' ? Math.max(1, this._page - this._pageOverdue) : 1;
	}
	...
	async _onPageChange(e) {
		window.D2L.Foundation.renderQueue = [];
		this.loading = true;
		await this.updateComplete;
		this._page = e.detail.page;
		await this._state.allFetchesComplete();
		while (window.D2L.Foundation.renderQueue.length > 0) {
			await window.D2L.Foundation.renderQueue.shift();
		}
		await this._state.allFetchesComplete();
		this.loading = false;
	}
